...
data <- data[c("genre" , "amabilite_1" , "amabilite_2", "article")]
...
```{r}
boxplot(data$amabilite_1 ~ data$genre*data$article)
```

```{r}
boxplot(data$amabilite_2 ~ data$genre*data$article)
```
...
describe(data$amabilite_1, skew = TRUE)
...
describe(data$amabilite_2, skew = TRUE)
```

# Valeurs extrêmes

```{r}
boxplot(data$amabilite_1)$out #Valeurs des VE
```
```{r}
boxplot(data$amabilite_1)$out #Valeurs des VE
```
...
```{r}
ggplot(data, aes(x=as.numeric(article), y=amabilite_1, fill=genre, color=genre)) + 
        geom_point(size = 1.5) + 
        labs(x = "article", y ="amabilite_1", colour = "Genre") +
